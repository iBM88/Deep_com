...
%   in tricky4: noPruning:fail, pruning:fail  
...
if(isPrune)
    ...
    fprintf('features=%d\tpattern=%d\tcommunities=%d\tisLoop=%d\tisPrune=%d\n',size(all_patterns,2),size(all_patterns,1),size(unique(all_coms),1),isLoop,isPrune);
else
    % No pruning
    ...
    fprintf('features=%d\tpatterns=%d\tcommunities=%d\tisLoop=%d\tisPrune=%d\n',size(all_patterns,2),size(all_patterns,1),size(unique(all_coms),1),isLoop,isPrune);
end
